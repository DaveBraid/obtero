...
new_method = '''  private showIdeaActionModal(idea: IdeaItem, parentModal: Modal): void {
    ...
    valhallaBtn.style.cssText = 'min-height: 38px; padding: 8px 20px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; border: 1px solid transparent; background: transparent; color: var(--text-error);';

    // 删除
    const deleteBtn = actions.createEl('button', { text: '删除' });
    deleteBtn.style.cssText = 'min-height: 38px; padding: 8px 20px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; border: 1px solid transparent; background: transparent; color: var(--text-muted);';
...
if start_idx and end_idx:
    ...
    print("✅ 已实现查看-编辑模式切换")
    print("  • 默认：只读查看模式")
    print("  • 点击 ✏️ 编辑：切换到编辑模式")
    print("  • 点击 💾 保存：保存并切回查看模式")
    print("  • 点击 取消：放弃修改并切回查看模式")
else:
    print("❌ 未找到方法")
