...
// @ts-ignore
  ...
  addFieldStyleSetting(containerEl: HTMLElement, field: FieldStyle, index: number): void {
    // 预览区域
    const preview = containerEl.createDiv({ cls: 'pm-field-preview' });
    ...
    this.updateFieldPreview(preview, field);
...
// @ts-ignore
            await this.plugin.saveSettings();
// @ts-ignore
            this.updateFieldPreview(preview, this.plugin.settings.fields[index]);
          });
      });
...
// @ts-ignore
        colorPicker
// @ts-ignore
          .setValue(field.textColor)
// @ts-ignore
// @ts-ignore
// @ts-ignore
          .onChange(async value => {
// @ts-ignore
            this.plugin.settings.fields[index].textColor = value;
            await this.plugin.saveSettings();
// @ts-ignore
            this.updateFieldPreview(preview, this.plugin.settings.fields[index]);
          });
...
// @ts-ignore
  ...
  updateFieldPreview(preview: HTMLElement, field: FieldStyle): void {
    ...
    preview.innerHTML = `
      <div style="color: ${field.textColor}; padding: 12px;">
        <strong style="font-size: 1.2em;">${field.name}</strong>
